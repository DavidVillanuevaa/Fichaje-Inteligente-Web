...
const LEGACY_MIGRATION_DONE_KEY = "fichaje_legacy_migration_done";
...
const I18N = {
  es: {
    ...
    editTaskDeleteHint: "Deja el nombre vacío para borrar esta tarea.",
    ...
    entryEditorDelete: "Borrar fichaje",
    entryEditorCancel: "Cancelar edición",
    entryUpdated: "Fichaje actualizado correctamente.",
    entryDeleteConfirm: "¿Seguro que quieres borrar este fichaje?",
    entryDeleted: "Fichaje borrado correctamente.",
  ...
  en: {
    ...
    editTaskDeleteHint: "Leave it empty to delete this task.",
    ...
    entryEditorDelete: "Delete log",
    entryEditorCancel: "Cancel edit",
    entryUpdated: "Time log updated successfully.",
    entryDeleteConfirm: "Are you sure you want to delete this time log?",
    entryDeleted: "Time log deleted successfully.",
...
const entryEditorDeleteBtn = document.getElementById("entryEditorDeleteBtn");
...
async function bootstrap() {
  ...
  entryEditorDeleteBtn.addEventListener("click", onEntryEditorDelete);
...
async function onEntryEditorDelete() {
  if (!currentEditingEntryId) return;
  const entry = yearEntries.find((item) => item.id === currentEditingEntryId);
  if (!entry) return;

  const confirmed = confirm(t("entryDeleteConfirm"));
  if (!confirmed) return;
...
async function handleEditTask(task) {
  const input = prompt(`${tf("editTaskPrompt", { task: task.name })}\n${t("editTaskDeleteHint")}`, task.name);
...
function applyTranslations() {
  ...
  setText("entryEditorDeleteBtn", t("entryEditorDelete"));
...
function drawTaskChart(entries) {
  ...
  const margin = { top: 42, right: 88, bottom: 42, left: 220 };
  const plotWidth = cssWidth - margin.left - margin.right;
  const plotHeight = cssHeight - margin.top - margin.bottom;
  const rawMaxHours = Math.max(...data.map((d) => d.hours), 1);
  const maxHours = niceCeil(rawMaxHours);
  const barGap = 12;
  const barHeight = Math.max(18, (plotHeight - (data.length - 1) * barGap) / data.length);
  const axisTicks = 5;
  const palette = ["#0f766e", "#1f8a7f", "#329c8d", "#43ae9a", "#56bea8", "#6fcbba", "#8ad7cb", "#a5e3dc", "#c0ece7", "#d7f3ef"];

  const panelGradient = ctx.createLinearGradient(0, margin.top, 0, margin.top + plotHeight);
  panelGradient.addColorStop(0, "#f8fcfa");
  panelGradient.addColorStop(1, "#eef7f3");
  ctx.fillStyle = panelGradient;
  ctx.fillRect(margin.left, margin.top, plotWidth, plotHeight);

  ctx.strokeStyle = "#d5e8df";
  ctx.lineWidth = 1;
  ctx.strokeRect(margin.left + 0.5, margin.top + 0.5, plotWidth - 1, plotHeight - 1);

  ctx.font = "12px IBM Plex Mono, monospace";
  ctx.fillStyle = "#5a786a";
  ctx.textBaseline = "middle";
  for (let i = 0; i <= axisTicks; i++) {
    const value = (maxHours / axisTicks) * i;
    const x = margin.left + (value / maxHours) * plotWidth;
    ctx.strokeStyle = i === 0 ? "#b6d8cc" : "#deebe5";
    ctx.beginPath();
    ctx.moveTo(x + 0.5, margin.top);
    ctx.lineTo(x + 0.5, margin.top + plotHeight);
    ctx.stroke();
    ctx.fillText(value.toFixed(0), x - 4, margin.top + plotHeight + 16);
  }

  ctx.font = "600 13px Space Grotesk, sans-serif";
  ctx.fillStyle = "#27483c";
  ctx.textBaseline = "alphabetic";
  ctx.fillText(`${activeYear} • ${data.length} ${lang === "es" ? "tareas" : "tasks"}`, margin.left, 28);

  data.forEach((item, index) => {
    const y = margin.top + index * (barHeight + barGap);
    const width = (item.hours / maxHours) * plotWidth;
    const radius = Math.min(7, barHeight / 2);

    const barGradient = ctx.createLinearGradient(margin.left, y, margin.left + width, y + barHeight);
    barGradient.addColorStop(0, palette[index % palette.length]);
    barGradient.addColorStop(1, "#0c5c56");
    ctx.fillStyle = barGradient;
    roundRectPath(ctx, margin.left, y, width, barHeight, radius);
    ctx.fill();

    ctx.fillStyle = "#10241b";
    ctx.font = "13px Space Grotesk, sans-serif";
    ctx.textBaseline = "middle";
    ctx.fillText(truncateLabel(item.task, 28), 14, y + barHeight / 2);
  ...
  ctx.fillStyle = "#4d7060";
  ctx.font = "12px IBM Plex Mono, monospace";
  ctx.textBaseline = "alphabetic";
  ctx.fillText(t("chartHours"), margin.left, cssHeight - 10);
}
